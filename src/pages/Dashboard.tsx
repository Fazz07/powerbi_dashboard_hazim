...
const Dashboard = () => {
  ...
  const sendChatMessage = async (
    ...
    if (!currentChatSessionId) {
       // Using crypto.randomUUID() for a standard, non-identifying unique ID
       ...
       if (!willBeOpen) { // Chatbot is being closed
           ...
           if (chatHistory.length > 0 && currentChatSessionId) {
               ...
               if (recentUserMessages.length > 0) { // Only save if there was at least one user message
                   ...
                   try {
                       // Note: getCurrentUserId() is still called here to get the actual user ID
                       // which is then derived on the backend from the token and used as partition key.
